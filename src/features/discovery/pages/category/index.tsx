...
export default function CategoryPage() {
  ...
  const { slug } = useParams({ from: "/_storefront/category/$slug" });
  const search = useSearch({ from: "/_storefront/category/$slug" });
  ...
  const navigateWithParams = (targetSlug: string, nextParams: ProductDiscoveryParams) => {
  ...
  const updateSearch = (updates: Partial<ProductDiscoveryParams>) => {
    const targetSlug = updates.category !== undefined ? updates.category : params.category;
    ...
    navigateWithParams(targetSlug ?? "", next);
  };
  ...
  if (!categoriesQuery.isLoading && !category) {
    return (
      <div className="flex min-h-96 flex-col items-center justify-center rounded-xl border border-gray-100 bg-white p-8 text-center shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <Icon icon="mdi:shape-outline" className="text-gray-300" width={72} />
        <h1 className="mt-4 text-xl font-bold text-gray-900 dark:text-gray-100">{t("category.notFound")}</h1>
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{t("category.notFoundDesc")}</p>
  ...
  return (
    <div className="space-y-5">
      <nav className="flex flex-wrap items-center gap-1 text-sm text-gray-500 dark:text-gray-400">
        ...
        <Link to="/category" className="hover:text-ocop">
          {t("category.title")}
        </Link>
        <Icon icon="mdi:chevron-right" width={16} />
        <span className="font-semibold text-gray-900 dark:text-gray-100">{category?.name ?? t("common.loading")}</span>
      </nav>
            ...
            <div className="flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center">
              <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  {category?.name ?? t("category.title")}
                </h1>
