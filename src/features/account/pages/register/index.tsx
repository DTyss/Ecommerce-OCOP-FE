...
export default function StorefrontRegisterPage() {
  ...
  const register = useStorefrontAuthStore((state) => state.register);
  ...
  return (
    <div className="relative isolate -mx-4 flex min-h-[calc(100vh-156px)] items-center justify-center overflow-hidden px-4 py-8 sm:-mx-6 lg:-mx-8">
      <section className="relative w-full max-w-[620px] rounded-2xl border border-white/70 bg-white/58 p-6 text-gray-900 shadow-[0_24px_80px_rgba(11,107,58,0.16)] backdrop-blur-2xl sm:p-8 dark:border-white/20 dark:bg-gray-950/40 dark:text-white">
        ...
        <div className="relative mb-7 text-center">
          <h1 className="text-2xl font-bold text-gray-950 dark:text-white">{t("auth.registerTitle")}</h1>
          <p className="mt-2 text-sm font-medium text-gray-600 dark:text-white/70">{t("auth.registerSubtitle")}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="relative space-y-4">
          <form.Field
            name="fullName"
            validators={{
              onChange: ({ value }) => (!value.trim() ? t("auth.fullNameRequired") : undefined),
            }}
          >
            {(field) => (
              <div className="space-y-1.5">
                <div className="relative">
                  <Icon icon="mdi:account-outline" className={authIconClass} width={20} />
                  <Input
                    id={field.name}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            >
              {(field) => (
                <div className="space-y-1.5">
                  <div className="relative">
                    <Icon icon="mdi:phone-outline" className={authIconClass} width={20} />
                    <Input
                      id={field.name}
                      value={field.state.value}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            >
              {(field) => (
                <div className="space-y-1.5">
                  <div className="relative">
                    <Icon icon="mdi:lock-check-outline" className={authIconClass} width={20} />
                    <Input
                      id={field.name}
          ...
          <label className="flex items-start gap-2 text-sm font-medium text-gray-700 dark:text-white/75">
            <Checkbox
              checked={acceptedTerms}
              onCheckedChange={(checked) => setAcceptedTerms(checked === true)}
              className="border-ocop/30 data-[state=checked]:border-ocop data-[state=checked]:bg-ocop mt-0.5 bg-white/70 dark:border-white/50 dark:bg-white/5"
            />
            <span>
              {t("auth.agreeToTerms")}{" "}
              <a
